...
namespace Lesson3_CNLTWeb.Controllers
{
    public class RoomBCS240032Controller : Controller
    {
        ...
        public async Task<IActionResult> Create(RoomBCS240032 room, string? imageUrl)
        {
            try
            {
                ...
                {
                    ModelState.AddModelError("imageUrl", "Vui lòng nhập đường dẫn ảnh đại diện");
                }
                else if (!IsValidImageUrl(imageUrl))
                {
                    ModelState.AddModelError("imageUrl", "Đường dẫn ảnh phải bắt đầu bằng http:// hoặc https://");
                }

                if (!ModelState.IsValid)
                {
                    await PopulateRoomTypesAsync(room.RoomTypeId);
                    return View(room);
                }
                ...
                await _roomImageRepository.AddAsync(new RoomImageBCS240032
                {
                    RoomId = room.Id,
                ...
                TempData["SuccessMessage"] = "Thêm phòng và ảnh đại diện thành công";
                return RedirectToAction(nameof(Index));
            }
        ...
        private async Task PopulateRoomTypesAsync(int? selectedRoomTypeId = null)
        {
            var roomTypes = await _roomTypeRepository.GetAllAsync();
